    ...
    <div className="bg-white dark:bg-[#0c101b] border border-slate-200 dark:border-[#1e293b] rounded-lg p-5 shadow-sm space-y-4">
      ...
      <div className="flex justify-between items-center pb-3 border-b border-slate-200 dark:border-[#1e293b]/60">
        <div className="space-y-0.5">
          ...
          <h3 className="text-xs font-bold text-slate-800 dark:text-slate-200">Chronological Telemetry Anomalies</h3>
        </div>
        
        {incidents.length > 0 && (
          <button 
            onClick={onExportJSON}
            className="flex items-center gap-1 px-2.5 py-1 border border-slate-200 dark:border-[#1e293b] bg-white dark:bg-[#0c101b] hover:bg-[#1a202c] text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white rounded transition-colors text-[9px] font-mono font-bold"
          >
        ...
        <div className="relative pl-4 space-y-4 py-1.5 max-h-[190px] overflow-y-auto pr-1">
          ...
          {incidents.map((inc) => (
            <div key={inc.id} className="relative z-10 flex gap-3.5 text-[10px] font-mono">
              ...
              {/* Chrono Node bullet */}
              <div className={`w-3 h-3 rounded-full border bg-white dark:bg-[#0c101b] mt-1 flex-shrink-0 ${
                inc.level === 'critical' 
              ...
              <div className="flex-1 space-y-1 bg-slate-50 dark:bg-[#080b11]/50 border border-slate-200 dark:border-[#1e293b]/40 rounded p-2.5">
                ...
                <h4 className="text-xs font-bold text-slate-900 dark:text-white font-sans mt-1">{inc.title}</h4>
                <p className="text-slate-600 dark:text-slate-400 font-sans font-light leading-relaxed">{inc.desc}</p>
                <div className="pt-1 text-[8px] text-slate-500">
                  Affected Service: <span className="text-slate-600 dark:text-slate-400 font-semibold">{inc.service}</span>
                </div>
              </div>
