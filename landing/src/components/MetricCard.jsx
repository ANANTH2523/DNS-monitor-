...
export default function MetricCard({ title, value, unit = '', desc, icon: Icon, isError = false, loading = false }) {
  return (
    <div className={`bg-[#0c101b] border rounded-lg p-4 flex flex-col justify-between shadow-sm relative group hover:border-[#334155] transition-all duration-300 ${
      isError ? 'border-rose-500/30' : 'border-[#1e293b]'
    }`}>
      ...
      <div>
        {loading ? (
          ...
          <p className={`text-2xl font-bold font-mono tracking-tight ${isError ? 'text-rose-500' : 'text-white'}`}>
